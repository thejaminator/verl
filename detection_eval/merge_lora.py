...
def load_model_from_hf(model_name: str, token: Optional[str] = None):
    ...
    try:
        ...
        raise ValueError(f"Expected a PEFT adapter repository at '{model_name}', but no PEFT config was found.") from e

    # Pick the repo to read config/tokenizer from (the base model specified by the adapter)
    cfg_source = base_model_name

    # Load model configuration first
    config = AutoConfig.from_pretrained(cfg_source, token=token, trust_remote_code=True)

    # Load tokenizer
    logger.info("Loading tokenizer...")
    tokenizer = AutoTokenizer.from_pretrained(cfg_source, token=token, trust_remote_code=True)

    # Load model with appropriate dtype
    logger.info("Loading model weights...")
    base_model = AutoModelForCausalLM.from_pretrained(
        cfg_source, config=config, token=token, torch_dtype=torch.bfloat16, device_map="auto", trust_remote_code=True
    )

    # Wrap with PEFT so we can merge later
    logger.info("Wrapping base model with PEFT adapter...")
    model = PeftModel.from_pretrained(base_model, model_name, token=token)
...
def check_for_lora_adapter(model_name: str, token: Optional[str] = None):
    ...
    try:
        ...
        for file in lora_files:
            try:
                hf_hub_download(repo_id=model_name, filename=file, token=token)
...
def main(
    source_model: str = typer.Option(
        "thejaminator/gemma-introspection-20250821", "--source-model", help="Source model identifier on HuggingFace"
    ),
    target_model: str = typer.Option(
        "thejaminator/gemma-introspection-20250821-merged", "--target-model", help="Target model identifier for upload"
    ),
    ...
    model, tokenizer, config = load_model_from_hf(source_model, token=hf_token)

    # Step 2: Check for and merge LoRA weights if present
    logger.info("Step 2: Checking for LoRA adapter...")
    has_lora = check_for_lora_adapter(source_model, token=hf_token)

    if has_lora:
        logger.info("LoRA adapter detected. Merging weights...")
        model = merge_lora_weights(model, source_model, token=hf_token)
    else:
    ...
    upload_to_hf(model, tokenizer, target_model, token=hf_token, private=private)
