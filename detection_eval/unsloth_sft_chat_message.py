"""
Unsloth-Optimized SFT Training Script

This script performs supervised fine-tuning (SFT) on conversation data using Unsloth
for faster training and inference.

Features:
- Unsloth's optimized model loading and training
- Automatic Hugging Face login at script start
- LoRA fine-tuning support with Unsloth optimizations
...
- Faster training and lower memory usage compared to standard transformers

Usage:
    python unsloth_sft_chat_message.py [conversations_file.jsonl]
a
Before running:
1. Make sure you're logged into Hugging Face: `huggingface-cli login`
2. Install Unsloth: `pip install unsloth`
3. Update the hf_repo_id in the main() function to your desired repository name
4. Ensure you have the required conversations JSONL file with FinetuneConversation objects
"""
...
class SFTTrainingConfig:
    ...
    load_in_4bit: bool = False  # Use 4-bit quantization with Unsloth
    continue_from_lora: bool = False  # Not supported with Unsloth yet
...
def load_model(
    ...
    print(f"Loading model with Unsloth: {cfg.model_name}...")
    
    # Determine max sequence length
    max_seq_length = cfg.max_tokens
    ...
    if use_lora:
        model, tokenizer = FastLanguageModel.from_pretrained(
            ...
            dtype=dtype,  # None = auto-detect, or torch.float16 / torch.bfloat16
        ...
        model = FastLanguageModel.get_peft_model(
            model,
            r=cfg.lora_r,
            target_modules=cfg.lora_target_modules.split(",") if isinstance(cfg.lora_target_modules, str) and cfg.lora_target_modules != "all-linear" else ["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"],
        ...
        print("✓ Successfully loaded model with LoRA using Unsloth")
        ...
        print("✓ Successfully loaded model using Unsloth (no LoRA)")

    return model, tokenizer


# Removed SAE-specific utility functions and has_active_lora (not needed with Unsloth)
...
def train_model(
    ...
    # Enable training mode for Unsloth model
    FastLanguageModel.for_training(model)

    # Use AdamW optimizer (Unsloth works well with standard optimizers)
...
def main(
    ...
    print(asdict(cfg))
    # Let Unsloth auto-detect the best dtype (None), or use torch.bfloat16/torch.float16
    dtype = None
    ...
    model, tokenizer = load_model(cfg, device, dtype, use_lora=cfg.use_lora)
...
if __name__ == "__main__":
    # Example usage with Unsloth optimizations:
    import datetime
