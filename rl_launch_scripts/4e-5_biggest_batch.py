...
if __name__ == "__main__":
    ...
    PARAMS = VerlParams(
        ...
        grad_clip=0.3,
        prompt_batch_size=128,  # number of prompts in rollout batch. will be multiplied by num_generations.
        # split_into_grad_accum=64,  # prompt_batch_size * num_generations gets split by grad accum.
        split_into_grad_accum=256,  # need for no padding
        ...
        warmup_steps=5,
        learning_rate=2e-5,  # Increased by order of magnitude for LoRA (was 5e-6)
        # learning_rate=5e-4,  # Increased by order of magnitude for LoRA (was 5e-6)
        entropy_coeff=0.001,
        loss_kl_penalty=0.000,  # Note: we are calculating KL w.r.t to the base model without SFT, which is weird.
        ...
        max_steps=4000,
        output_dir="/workspace/12sep_grp16_4e5_lr_batch_128",
        ...
        push_to_hub=True,
        hub_repo_id="thejaminator/12sep_grp16_4e5_lr_batch_128",  # Updated with "_verl" suffix
